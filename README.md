# QA Challenges V1.0 Task 1 and V2.2 Task 1
Please open the installation guide for setup, the following is my reference library from 
 - com.restfulbooker.apitest Framework README file. 
...
> Create a `CryptoCandleTest.java` file under `com.restfulbooker.apitest.businessLogics` package. (`src/test/java/` package holds all test classes (TestNG) related to application.)
```
...
public class CryptoCandleTest {
  ...
  @Test
  public void validCryptoCandleTest(Method method) {
  ...
  @Test
  public void invalidCryptoCandleTest(Method method) {
...
> Similarly in `invalidCryptoCandleTest` method, we are asserting whether the response body data `reason` returns as `Bad credentials` or not.
